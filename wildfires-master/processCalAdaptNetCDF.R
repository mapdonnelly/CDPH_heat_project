...
flist <- list.files( path="~/Desktop/GitHub/CDPH_heat_project/data/", pattern = "^.*\\.(nc|NC|Nc|Nc)$")
flist<-as.list(flist)
#flist <- c(
...
process_nc <- function(file){
    ...
    #   i<-1
      ...
      ##close the netcdf file -> not sure why....
      nc_close(ncin)
      ...
      # check number of missing data values
       ...
       dimnames(tmp_array) <-list(lon=nc_lon, lat=nc_lat, day=nc_day)
      
       ##look at a slice
       m<-1
       tmp.slice<-tmp_array[,,m]
       image(nc_lon, nc_lat, tmp.slice, col = rev(brewer.pal(10, "RdBu")))
       
       
      bar <- data.frame(expand.grid(lon = nc_lon, lat = nc_lat)) %>%
        cbind(matrix(as.vector(tmp_array), nrow = n_lon*n_lat, ncol = n_day)) %>%
        mutate(model=unlist(str_split(string = file, pattern = "_"))[1],
               scenario = unlist(str_split(string = file, pattern = "_"))[2],
               temp = unlist(str_split(string = file, pattern = "_"))[3],
               year_start = unlist(str_split(string = file, pattern = "_"))[4])
      
      ##reorder the columns to put model info at beginning
        bar<-  bar[,c(368:371,1,2,3:367)]
        names(bar) <- c("model","scenario","temp","year_start", "lon","lat",1:365) 
    
      return(bar)
      # # set the name of my new variable and bind the new data to it
      # if(exists("tasmax")){
      #   wildfire <- bind_rows(tasmax, bar)
      # }else{
      #   tasmax <- bar
      # }
      # tidy up, not sure if necesarry really, but neater
      rm(nc_day, nc_lat, nc_lon, ncin, bar)
...
test<-lapply(flist, process_nc)

keycols <- c("lon", "lat")
test<- setkeyv(test, keycols)
